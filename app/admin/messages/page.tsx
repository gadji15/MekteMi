...
import { Bell, Send, Plus, Clock, Edit, Trash2, MoreHorizontal, X, AlertCircle, Search } from "lucide-react"
...
export default function AdminMessagesPage() {
  ...
  const [form, setForm] = useState<{ title: string; message: string; type: MsgType }>({
    title: "",
    message: "",
    type: "info",
  })
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-lg">Envoyez et gérez les messages adressés aux pèlerins</p>
        </div>
        <div className="flex gap-3">
          ...
          <Button onClick={startCreate} className="cursor-pointer bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary transition-all duration-300 hover:scale-105">
      ...
      <div className="space-y-4">
        {filtered.map((n) => (
          <Card key={n.id} className="hover:shadow-lg transition-all duration-300 hover:scale-[1.01] bg-gradient-to-br from-card to-muted/30 border-0">
      ...
      {showForm && (
        <Card className="bg-gradient-to-br from-card to-muted/30 border-0 shadow-lg animate-slide-up">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-2xl">{editing ? "Modifier le message" : "Nouveau message"}</CardTitle>
              <CardDescription>
                {editing ? "Mettez à jour le message" : "Rédigez et envoyez un nouveau message aux pèlerins"}
              </CardDescription>
            </div>
            <Button
              variant="ghost"
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <Select value={form.type} onValueChange={(value: MsgType) => setForm((p) => ({ ...p, type: value }))} disabled={submitting}>
